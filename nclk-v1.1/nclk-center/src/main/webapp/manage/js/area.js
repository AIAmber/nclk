require(['jquery','ztree','Util','bootstrap-dialog','bootstrap-grid','table-toolbar','jquery-placeholder'], function($,ztree,Util,BootstrapDialog,placeholder) {
	...
	$("#tree-update").on("click",function(){
		...
		$.ajax({
		    ...
		    success: function(data){
		    	modelContentEdit.find("#areaId_edit").val(data.AREA_ID);
		    	modelContentEdit.find("#areaName_edit").val(data.AREA_NAME);
		    	modelContentEdit.find("#areaType_edit").val(data.AREA_TYPE);
		    	modelContentEdit.find("#ord_edit").val(data.ORD);
		    	modelContentEdit.find("input[name='state_edit'][value='"+data.STATE+"']").prop("checked",true);
		    	editStateValue=data.STATE;
		    	var parentId=data.PARENT_AREA_ID;
		    	if(data.AREA_TYPE=="1"){
		    		modelContentEdit.find("#upArea_edit").empty();
		    		modelContentEdit.find("#upArea_edit").attr("disabled",true);
		    	}else {
		    		 $.ajax({
		                 url: Util.getPath()+"/AreaController/findAreaInfoByAreaType",
		                 type: 'post',
		                 ...
		                 success: function (data) {
		                	 	   modelContentEdit.find("#upArea_edit").removeAttr("disabled");
		                	 	   modelContentEdit.find("#upArea_edit").empty();
		                	 	   modelContentEdit.find("#upArea_edit").append("<option value='' >请选择上级区域</option>");
		                           for(var i=0;i<data.length;i++){
		                        	   modelContentEdit.find("#upArea_edit").append("<option value='"+data[i].AREA_ID+"'>"+data[i].AREA_NAME+"</option>");
		                           }
		                           modelContentEdit.find("#upArea_edit option[value="+parentId+"]").attr("selected",true);
		                	 }
		                 });
		    	}
		    }
	...
	//区域模态框信息保存
	var modelContentEdit=$('#modelContentEdit');
	var dialogEdit = new BootstrapDialog({
        message: modelContentEdit,
        type:'type-default',
        ...
        buttons: [{
            ...
            action: function(dialogRef){
            	if(!editInfoForm.valid()){
            		...
            		var areaId=$("#areaId_edit").val();
            		var state = $("input[name='state_edit']:checked").val();
        			//修改区域状态从X到A
	...
	//区域模态框信息新增
	var modelContentAdd=$('#modelContentAdd');
	var dialogAdd = new BootstrapDialog({
        message: modelContentAdd,
        type:'type-default',
        closeByBackdrop: false,
        closeByKeyboard: false,
        autodestroy: false,
        ...
        buttons: [{
            ...
            action: function(dialogRef){
            	if(!addInfoForm.valid()){
            		...
            		var areaId=$("#areaId_add").val();
            		var state = $("input[name='state_add']:checked").val();
            		//区域新增
        			$.ajax({
            		    method: 'post',
            		    ...
            		    dataType: 'json',
            		    data:{
            		    	areaId:$("#areaId_add").val(),
            		    	areaName:$("#areaName_add").val(),
            		    	areaType:$("#areaType_add").val(),
            		    	parentId:$("#upArea_add").val(),
	...
	function saveFormInfo(areaId,dialogRef){
    	$.ajax({
		    ...
		    data:{
		    	...
		    	areaType:$("#areaType_edit").val(),
		    	parentId:$("#upArea_edit").val(),
