require(['jquery','bootstrap-grid','Util','table-toolbar','bootstrap-dialog','ztree','jquery-placeholder'],function($,bootstrapGrid,Util,toolbar,bootstrapDialog,ztree,placeholder){
	...
	$.ajax({
		...
		$.ajax({
	     	   ...
	     	   success: function (data) {
	     		  ...
	     		  //添加完成之后触发选择事件
		  	      $("#province").change();
		  	   	  $("#areaProvince_add").change();
		  	      $("#areaProvince_edit").change();
	     	   }
	     	});
        ...
        buttons: [{
            ...
            action: function(dialogRef){
                if(!exampointerAddInfo.valid()){
                	...
                	var endPointId=$("#examID_add").val();
            		$.ajax({
                        method: 'post',
                        url: Util.getPath()+"/EndPointController/findEndPointNo",
                        dataType: 'json',
                        data: {
                            endPointNum:$("#examNum_add").val(),
                        },
                        success: function(data){
	                        	//新增考点编号检验是否存在
	                        	if(data==0){
            ...
            action: function(dialogRef){
            	$('#exampointerAddForm')[0].reset();
                validatorAdd.resetForm();
                $('#areaProvince_add').children('option').first().prop("selected","selected");
                $("#areaCity_add").empty();
                $("#areaCity_add").prepend("<option value='' selected>请选择市</option>");
                $("#areaDistrict_add").empty();
                $("#areaDistrict_add").prepend("<option value='' selected>请选择县区</option>");
                dialogRef.close();
        ...
        buttons: [{
            ...
            action: function(dialogRef){
                if(!exampointerEditInfo.valid()){
                	...
                	var endPointId=$("#examID_edit").val();
                	var endPointerNum=$("#examNum_edit").val();
            ...
            cssClass:' btn-default btn-sm',
            action: function(dialogRef){
            	$('#exampointerEditForm')[0].reset();
                validatorEdit.resetForm();
                $('#areaProvince_edit').children('option').first().prop("selected","selected");
                $("#areaCity_edit").empty();
                $("#areaCity_edit").prepend("<option value='' selected>请选择市</option>");
                $("#areaDistrict_edit").empty();
                $("#areaDistrict_edit").prepend("<option value='' selected>请选择县区</option>");
                totalExamNumEdit="";
                dialogRef.close();
    ...
    $('#btn-add').on('click',function(){
    	 $('#areaProvince_add').children('option').first().prop("selected",true);
         $("#areaCity_add").empty();
         $("#areaCity_add").prepend("<option value='' selected>请选择市</option>");
         $("#areaDistrict_add").empty();
         $("#areaDistrict_add").prepend("<option value='' selected>请选择县区</option>");
        $(".icon-connect_add").show();
                             ...
                             success: function(data0){
                                 ...
                                 for(var i=0;i<data0.bean.length;i++){
                                     if(data0.bean[i].AREA_ID==areaId){
                                         $("#areaDistrict_edit").append("<option value='"+data0.bean[i].AREA_ID+"' selected='selected'>"+data0.bean[i].AREA_NAME+"</option>");
                                     }else{
                                    	 $("#areaDistrict_edit").append("<option value='"+data0.bean[i].AREA_ID+"'>"+data0.bean[i].AREA_NAME+"</option>");
                                     }
                                 }
                                 $.ajax({
                                     method: 'post',
                                             ...
                                             success: function(data2){
                                                 ...
                                                 for(var i=0;i<data2.bean.length;i++){
                                                     if(data2.bean[i].AREA_ID==districtParentId){
                                                         $("#areaCity_edit").append("<option value='"+data2.bean[i].AREA_ID+"' selected='selected'>"+data2.bean[i].AREA_NAME+"</option>");
                                                     }else{
                                                    	 $("#areaCity_edit").append("<option value='"+data2.bean[i].AREA_ID+"'>"+data2.bean[i].AREA_NAME+"</option>");
                                                     }
                                                 }
                                                 $.ajax({
                                                     method: 'post',
                                                     ...
                                                     success: function(data3){
                                                    	 $("#areaProvince_edit").empty();
                                                    	 $("#areaProvince_edit").append("<option value=''>请选择省</option>");
                                                    	 $("#areaProvince_edit").append("<option value='"+data3.AREA_ID+" ' selected='selected'>"+data3.AREA_NAME+"</option>");
    ...
    //给省份的选择框添加事件
    $("#areaProvince_add").change(function(){
        //获取选择的省份id
        var parentAreaId = $(this).val();
        $.ajax({
            url: Util.getPath()+"/AreaController/getCity",
            ...
            success: function (data) {
                $("#areaCity_add").empty();
                $("#areaCity_add").append("<option value='' selected>请选择市</option>");
                for (var i = 0; i < data.length; i++) {
                    $("#areaCity_add").append("<option value='"+data[i].AREA_ID+"'>"+data[i].AREA_NAME+"</option>");
                }
                //添加完成之后触发选择事件
                $("#areaCity_add").change();
            }
    ...
    //给地级市的选择框添加事件
    $("#areaCity_add").change(function(){
        //获取选择的地级市id
        var parentAreaId = $("#areaCity_add").val();
        $.ajax({
            url: Util.getPath()+"/AreaController/getDistrict",
            type: 'post',
            dataType: 'json',
            ...
            success: function (data) {
                $("#areaDistrict_add").empty();
                $("#areaDistrict_add").append("<option value='' selected>请选择县区</option>");
                if(null != data && data != undefined && data != ""){
                    var i = 0;
                    for (i = 0; i < data.length ; i++) {
                        $("#areaDistrict_add").append("<option value='"+data[i].AREA_ID+"'>"+data[i].AREA_NAME+"</option>");
                    }
                }
            }
        });
    ...
    //给省份的选择框添加事件
    $("#areaProvince_edit").change(function(){
        //获取选择的省份id
        var parentAreaId = $(this).val();
        $.ajax({
            url: Util.getPath()+"/AreaController/getCity",
            ...
            success: function (data) {
                $("#areaCity_edit").empty();
                $("#areaCity_edit").append("<option value='' selected>请选择市</option>");
                for (var i = 0; i < data.length; i++) {
                    $("#areaCity_edit").append("<option value='"+data[i].AREA_ID+"'>"+data[i].AREA_NAME+"</option>");
                }
                //添加完成之后触发选择事件
                $("#areaCity_edit").change();
            }
        });
    });

    //给地级市的选择框添加事件
    $("#areaCity_edit").change(function(){
        //获取选择的地级市id
        var parentAreaId = $("#areaCity_edit").val();
        $.ajax({
            url: Util.getPath()+"/AreaController/getDistrict",
            type: 'post',
            dataType: 'json',
            ...
            success: function (data) {
                $("#areaDistrict_edit").empty();
                $("#areaDistrict_edit").append("<option value='' selected>请选择县区</option>");
                if(null != data && data != undefined && data != ""){
                    var i = 0;
                    for (i = 0; i < data.length ; i++) {
                        $("#areaDistrict_edit").append("<option value='"+data[i].AREA_ID+"'>"+data[i].AREA_NAME+"</option>");
                    }
                }
            }
        });
    ...
    //新增表单获取考点编号按钮
    $("#test-connect-add").tooltip();
    $("#icon-connect-add").click(function(){
    	getPointerNum("add");
    });
  //编辑表单获取考点编号按钮
    $("#test-connect-edit").tooltip();
    $("#icon-connect-edit").click(function(){
    	getPointerNum("edit");
    });
    ...
    function getPointerNum(type){
        ...
        $.ajax({
            ...
            dataType:"json",
            data:{
                endPointIPAddr : $("#examIPAddress_"+type).val(),
                endPointPort : $("#examPort_"+type).val()
            },
            success:function(data){
            	$this.next().hide();
     			$this.show();
                if(data.success==true){
                    $("#examNum_"+type).val(data.bean);
                    Util.promptSuccessDialog("获取考点编号成功！");
                    $("#examNum_"+type).attr("readonly","readonly");
                }else{
                    Util.warningDialog("获取失败，请手动输入！");
                    $("#examNum_"+type).removeAttr("readonly");
                }
            },
  ...
  //考点信息初始化弹出框
	...
	function addPointerForm(endPointId){
		$.ajax({
            ...
            data: {
                endPointId:endPointId,
                endPointName:$("#examName_add").val(),
                endPointNum:$("#examNum_add").val(),
                endPointAreaId:$("#areaDistrict_add").val(),
                endPointAddr:$("#examAddress_add").val(),
                endPointPerson:$("#examPerson_add").val(),
                endPointTel:$("#examPhone_add").val(),
                endPointIPAddr:$("#examIPAddress_add").val(),
                endPointPort:$("#examPort_add").val(),
                state:$("input[name='state_add']:checked").val(),
                ord:$("#ord_add").val()
            },
            success: function(data){
                if (data.success) {
                ...
                $('#areaProvince_add').children('option').first().attr("selected","selected");
                $("#areaCity_add").empty();
                $("#areaCity_add").prepend("<option value='' selected>请选择市</option>");
                $("#areaDistrict_add").empty();
                $("#areaDistrict_add").prepend("<option value='' selected>请选择县区</option>");
	...
	function editPointerForm(endPointId){
		$.ajax({
            ...
            data: {
                endPointId:endPointId,
                endPointName:$("#examName_edit").val(),
                endPointNum:$("#examNum_edit").val(),
                endPointAreaId:$("#areaDistrict_edit").val(),
                endPointAddr:$("#examAddress_edit").val(),
                endPointPerson:$("#examPerson_edit").val(),
                endPointTel:$("#examPhone_edit").val(),
                endPointIPAddr:$("#examIPAddress_edit").val(),
                endPointPort:$("#examPort_edit").val(),
                state:$("input[name='state_edit']:checked").val(),
                ord:$("#ord_edit").val()
            },
            success: function(data){
                if (data.success) {
                ...
                $('#areaProvince_edit').children('option').first().attr("selected","selected");
                $("#areaCity_edit").empty();
                $("#areaCity_edit").prepend("<option value='' selected>请选择市</option>");
                $("#areaDistrict_edit").empty();
                $("#areaDistrict_edit").prepend("<option value='' selected>请选择县区</option>");
