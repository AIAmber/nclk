define(['bootstrap-dialog','jquery-validate','jquery-validate-messages-zh','daterangepicker-zh-CN'],function(BootstrapDialog){
	...
	return {
			...
			warningDialog:function(message,func,options){
			...
			dashboardErrorDialog:function(message){
				var prompt = $('<div class="promptError" style="text-align: center;"><span class="promptInfo">'+message+'</span></div>');
				if($('.mainPrompt').length==0||$('mainPrompt')==undefined){
					$('body').append('<div class="mainPrompt"></div>');
				}
				$('.mainPrompt').append(prompt);
			...
			datecustomRangePicker:function(options){
				$(".dateCustom-wrapper .dateCustom-remove").click(function(){
					$(this).parent().find(".dateCustom").val("");
				});
				var datePicker = $(".dateCustom-wrapper .dateCustom").daterangepicker($.extend({
                ...
                },options)).on('apply.daterangepicker',function(ev, picker){
