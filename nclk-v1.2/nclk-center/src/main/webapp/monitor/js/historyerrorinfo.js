require(['jquery', 'Util', 'moment', 'ztree', 'bootstrap-grid', 'table-toolbar', 'daterangepicker-zh-CN', 'jquery-placeholder'], function ($, Util, moment) {
    ...
    var examroomId = DATA.params.examroomId;
    ...
    var gridTable = $('#grids').bootstrapGrid({
        url: Util.getPath() + "/MonitorController/findHistoryErrorInfo?examroomId="+examroomId,
        uniqueId: 'EXAMROOM_LOG_HIS_ID',
        extraHeight: 20,
        columns: [{
            checkbox: true
        }, {
            ...
            title: '操作',
            formatter: function (value, row, index) {
                return '<a data-id="' + value + '" class="error-detail" href="javascript:void(0)">详情</a>';
            }
        }]
    });
