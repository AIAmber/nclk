define(['jquery', 'Router','Util'], function ($, Route,Util) {
    ...
    return {
        inint: function () {
            ...
            //运行监控-->历史异常信息查询
            var historyerrorinfo = function () {
                $("#headnavigation").empty();
                $("#headnavigation").append("&nbsp;&nbsp;<span>运行监控&nbsp;/异常信息查询&nbsp;/&nbsp;历史异常信息查询</span>");
                $('#rightContent').load("monitor/historyerrorinfo.jsp", null, function (response, status) {
                    if (status == "success") {
                        document.getElementById('rightContent').innerHTML = response;
                    }
                });
            ...
            var routes = {
                ...
                '/errorinfo': errorinfo,
                '/historyerrorinfo': historyerrorinfo,
