$(function() {
	...
	if(window.top!=window.self){
		window.top.location=Util.getPath()+'/sparknet/manage/login/index.jsp';
	}
...
function login(){
	...
	if(username==null||Util.trim(username.value)==''||username.value=='请输入用户名'){
		errorinfo.innerHTML='<img src="'+Util.getPath()+'/sparknet/manage/ui/icons/error.png"/> 请输入用户名';
		return false;
	}
	if(password==null||Util.trim(password.value)==''||password.value=='请输入密码'){
		errorinfo.innerHTML='<img src="'+Util.getPath()+'/sparknet/manage/ui/icons/error.png"/> 请输入密码';
		return false;
	}
	...
	if(captcha!=undefined&&captcha!=null){
		if(captcha==null||Util.trim(captcha.value)==''||captcha.value=='请输入验证码'){
			errorinfo.innerHTML='<img src="'+Util.getPath()+'/sparknet/manage/ui/icons/error.png"/> 请输入验证码';
			return false;
		}
	}
	//Cookie赋值
